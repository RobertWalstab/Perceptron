...
r_1         =   0.980;                          ''' coupling ratio1'''
r_2         =   0.972;                          ''' coupling ratio2'''
psi_b       =  -0.0628;                         ''' additional phase bias'''
...
P_in        =   np.arange(0, 15, 0.02);         ''' Input Power, discredited it certain steps'''
...
coeff_2[3] = (-(1 - a * r_2) ** 2 * phi_0 - ((2 * np.pi * L * n_2) / (lmda * A)) * a ** 2 * (
                1 - r_1 ** 2) * A * P_in[i] / (A_eff * 1000));
...
for ii in range(0, 2, 1):
    ...
    exp = t_1[i] - exp * t_2[i];
    P_out = (A * (1 - A) * P_in[i] * np.absolute(exp)**2);
